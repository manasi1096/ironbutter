...
# Sell OTM offset - sell 1 strike OTM instead of ATM for higher win rate
# 0 = sell ATM (old behavior), 50 = sell 1 strike OTM (new default)
SELL_OTM_OFFSET = int(os.getenv('SELL_OTM_OFFSET', 50))

# Trailing stop - once profit hits this % of credit, move stop to breakeven
TRAILING_STOP_TRIGGER_PCT = float(os.getenv('TRAILING_STOP_TRIGGER_PCT', 30))

# Trend filter - number of days to check for market direction
TREND_LOOKBACK_DAYS = int(os.getenv('TREND_LOOKBACK_DAYS', 5))
...
def get_market_trend(kite):
    """
    Check NIFTY's direction over the last N trading days.
    ...
    try:
        ...
        spot_symbol = f"NSE:{TRADING_SYMBOL} 50" if TRADING_SYMBOL == "NIFTY" else f"NSE:{TRADING_SYMBOL}"
        to_date = date.today()
        from_date = to_date - timedelta(days=TREND_LOOKBACK_DAYS + 5)  # extra days for weekends
        
        historical = retry_api_call(
        ...
        if len(historical) < 2:
            logger.warning("Not enough historical data for trend filter")
            return 'NEUTRAL'
        
        recent = historical[-TREND_LOOKBACK_DAYS:] if len(historical) >= TREND_LOOKBACK_DAYS else historical
        start_close = recent[0]['close']
        end_close = recent[-1]['close']
        change = end_close - start_close
        change_pct = (change / start_close) * 100
        
        logger.info(f"Trend ({TREND_LOOKBACK_DAYS}d): {start_close:.0f} → {end_close:.0f} ({change:+.0f} pts, {change_pct:+.2f}%)")
        
        if change_pct > 0.5:
            logger.info("Trend: BULLISH → favor PUT spreads")
            return 'BULLISH'
        elif change_pct < -0.5:
            logger.info("Trend: BEARISH → favor CALL spreads")
            return 'BEARISH'
        else:
            logger.info("Trend: NEUTRAL → use gap logic")
            return 'NEUTRAL'
    except Exception as e:
        logger.warning(f"Trend filter failed: {e}, defaulting to NEUTRAL")
        return 'NEUTRAL'
...
def execute_credit_spread():
    ...
    try:
        ...
        # Filter 4: Trend filter - override spread type if trend is strong
        trend = get_market_trend(kite)
        original_spread_type = spread_type
        if trend == 'BULLISH':
            spread_type = 'PUT'
            if original_spread_type != spread_type:
                logger.info(f"Trend override: {original_spread_type} → PUT (market is bullish)")
        elif trend == 'BEARISH':
            spread_type = 'CALL'
            if original_spread_type != spread_type:
                logger.info(f"Trend override: {original_spread_type} → CALL (market is bearish)")
        
        # Calculate ATM strike
        atm_strike = get_atm_strike(current_price, TRADING_SYMBOL)
        
        # Apply OTM offset - sell 1 strike OTM for higher win rate
        if SELL_OTM_OFFSET > 0:
            if spread_type == 'PUT':
                sell_strike = atm_strike - SELL_OTM_OFFSET
            else:
                sell_strike = atm_strike + SELL_OTM_OFFSET
            logger.info(f"ATM: {atm_strike} → Selling {SELL_OTM_OFFSET}pts OTM at {sell_strike}")
        else:
            sell_strike = atm_strike
        
        logger.info(f"Sell Strike: {sell_strike}")
        
        # Get option symbols using sell_strike (with retry)
        options = get_option_symbols(kite, sell_strike, spread_type)
        ...
        position = {
            ...
            "expiry": expiry,
            "atm_strike": sell_strike,
            "otm_strike": otm_strike,
            "hedge_distance": HEDGE_DISTANCE,
            "sell_otm_offset": SELL_OTM_OFFSET,
            "trend": trend,
            "gap": gap,
        ...
        if TELEGRAM_AVAILABLE:
            ...
            message = (
                ...
                f"🎯 <b>Sell Strike:</b> {sell_strike} (ATM: {atm_strike})\n"
                f"📍 <b>Expiry:</b> {expiry}\n\n"
                f"<b>Legs:</b>\n"
                f"  • Sell {sell_strike} {spread_type[0]}E @ ₹{atm_fill_price:.1f}\n"
...
def monitor_and_exit():
    ...
    try:
        ...
        trailing_stop_trigger = total_credit * (TRAILING_STOP_TRIGGER_PCT / 100)
        trailing_stop_active = position.get('trailing_stop_active', False)
        
        # Activate trailing stop once profit exceeds trigger threshold
        if not trailing_stop_active and unrealized_pnl >= trailing_stop_trigger:
            trailing_stop_active = True
            position['trailing_stop_active'] = True
            with open(POSITION_FILE, 'w') as f:
                json.dump(position, f, indent=2)
            logger.info(f"TRAILING STOP ACTIVATED - profit hit ₹{unrealized_pnl:.0f} (trigger: ₹{trailing_stop_trigger:.0f})")
        
        # Determine effective stop-loss level
        if trailing_stop_active:
            effective_stop = 0  # breakeven
            stop_label = "TRAILING (breakeven)"
        else:
            effective_stop = -stop_loss_threshold
            stop_label = f"₹{stop_loss_threshold:.0f} ({STOP_LOSS_PERCENT:.0f}% of max loss)"
        ...
        logger.info(f"Stop Loss: {stop_label} | Trailing: {'ON' if trailing_stop_active else 'OFF'}")
        ...
        if trailing_stop_active and unrealized_pnl <= 0:
            should_exit = True
            exit_reason = f"TRAILING STOP - Was profitable, hit breakeven (P&L: ₹{unrealized_pnl:.0f})"
        elif not trailing_stop_active and unrealized_pnl <= -stop_loss_threshold:
            should_exit = True
